...
export default {
  ...
  methods: {
    ...
    getDataInfo () {
      axios.get('/api/strategy/get.html?id=' + this.$route.params.id)
          .then(this.handleGetStrSucc.bind(this))
          .catch(this.handleGetDataErr.bind(this))
    },
    handleGetStrSucc (res) {
      if (res && res.data.data) {
        this.detail = res.data.data.strategy
        this.date = res.data.data.strategy.createdDate
      }
    },
    handleGetDataErr () {
      alert('服务器发生错误！')
